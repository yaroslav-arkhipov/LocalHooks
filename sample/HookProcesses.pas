...
type TCustomEditClass = class(Vcl.StdCtrls.TCustomEdit);
...
// Hook DoSetTextHint procedure of TCustomEdit class
procedure DoSetTextHint(ASelf: TObject; const Value: String);
var
  LEdit: TCustomEditClass;
begin
  LEdit := TCustomEditClass(ASelf);

  if CheckWin32Version(5, 1) and TCustomEditClass(ASelf).HandleAllocated then
    SendTextMessage(TCustomEditClass(ASelf).Handle, EM_SETCUEBANNER, WPARAM(1), LEdit.TextHint);
end;
//
//procedure HookPaint;
//begin
//  PaintMethodBackup := HookProc(@TCustomEditClass.DoSetTextHint, @DoSetTextHint);
//end;
//
//procedure UnHookPaint;
//begin
//  UnhookProc(@TCustomEditClass.DoSetTextHint, PaintMethodBackup);
//end;

initialization
  TBaseHook.ReplaceProcedure(@TCustomEditClass.DoSetTextHint, @DoSetTextHint);
//  HookPaint;
finalization
  TBaseHook.ReplaceProcedure(@DoSetTextHint, @TCustomEditClass.DoSetTextHint);
  //UnHookPaint;
end.
